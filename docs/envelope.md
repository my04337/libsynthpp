...
本シンセサイザでは **AHDSFR** 方式のエンベロープジェネレータを使用しています。
...
ドラムパートではノートオフを無視し、Decay 完了後に `threshold_level` 以下になった時点で
自動的に止音 (Free) へ遷移します。Release/Sustain/Fade は使用しません。
...
CC 72 (Release Time) は発音中のボイスにリアルタイムで反映されます。

- **ボイス生成時**: 楽器定義のベースリリースタイムを `Voice::mBaseReleaseTimeSec` に保存
- **CC 72 変更時**: `MidiChannel::updateReleaseTime()` → `Voice::setReleaseTimeScale()` → `EnvelopeGenerator::setReleaseTime()`
...
スケーリング後、以下を行います:
1. Voice に `baseReleaseTime` (スケーリング前の `r`) を保存
2. `EnvelopeGenerator::setMelodyEnvelope()` にスケーリング後の最終値を設定
...
| パラメータ | 計算式 |
|-----------|--------|
| Attack | `a × calcEGTimeScale(CC73)` |
| Decay | `d × calcEGTimeScale(CC75)` |

ドラムは AHD モデルのため、Release Time (CC 72) は適用されません。

### 5.3 CC 72 リアルタイム更新フロー

CC 72 の受信、または NRPN (1, 102) の変更をトリガーとして、以下の手順で発音中のボイスに反映します。
...
| 4 | `EnvelopeGenerator::setReleaseTime()` | EG のリリースタイムを更新。Release 中の場合は進行度を維持して時間を補正 |
...
| ファイル | 内容 |
|---------|------|
| `lsp/dsp/envelope_generator.hpp` | `EnvelopeGenerator` クラス (AHDSFR エンベロープ) |
| `lsp/synth/voice.hpp` / `.cpp` | `Voice` 基底クラス (EG 保持、Hold/Sostenuto/Release 制御) |
