...
void Test::SDLTest::exec()
{
	// Message
	Log::d("Testing : SDL");
	{
		Audio::SDLOutput wo(44100, 2, Audio::SDLOutput::SampleFormat::Float32);
		if (!wo.valid()) {
			Log::d("Testing : SDL - failed (open)");
			return;
		}
		...
		if(!wo.start()) {
			Log::w("Testing : SDL - failed (start)");
			return;
		}
		while(true) {
		...
		if(!wo.stop()) {
			Log::w("Testing : SDL - failed (stop)");
			return;
		}
	}

	Log::d("Testing : SDL - End");
}